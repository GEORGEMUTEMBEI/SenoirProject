# Importing Tkinter and other reqiured Libraries and models 
from tkinter import *
import cv2
import os
...
def login():
    if username_var.get() == "" or password_var.get() == "":
        ...
        try:
            ...
            row = curr.fetchone()
            if row == None:
                messagebox.showerror('Error','Invalid Data, PLEASE ENTER CORRECT USERNAME AND PASSWORD:)')
                ...
                def manage_student():
                    try:
                        ...
                        background_pic.pack()
                        first.title("Manage Students mojar")
                        print("Hello George Welcome........")
                        face = Label(first, text = "Student Management " , bg = "skyblue" , fg = "black", padx = 15, pady = 15, font = ("Times New Roman", 20, "bold") ,borderwidth = 5, relief = RIDGE).place(x = 500, y = 10)
                        main = Label(first, bg = "skyblue", borderwidth = 1).pack()
                        def back():
                            first.destroy() 
                        backbtn = Button(first, text = 'Back', font = ('Times new Roman', 15), fg = 'black', bg = 'skyblue', height = 1, width = 7, command = back).place(x = 1250, y = 10)  
                        ...
                        def add_student():
                            ...
                            if mojar_var.get() == "" or fname_var.get() == "" or gender_var.get() ==  "" or contact_var.get() == "" or address_var.get() == "":
                                        ...
                                        if (re.search('', contact_var.get())):
                                            ...
                                            if(re.search(regex, address_var.get())):
                                                ...
                                                if not os.path.exists(input_directory):
                                                    ...
                                                    cur1 = conn.cursor()
                                                    cur1.execute("insert into attendance(mojar,fname,gender,contact_no,email_address,date_of_join) VALUES (%s,%s,%s,%s,%s,%s)", (
                                                                                                                                                                                mojar_var.get(),
                                                                                                                                                                                fname_var.get(),
                                                                                                                                                                                gender_var.get(),
                                                                                                                                                                                contact_var.get(),
                                                    ...
                                                    (id,) = output
                                                    os.rename(os.path.join(dataset_dir,name),os.path.join(dataset_dir,name))
                                                    display()
                                                    clear()
                                                    conn.close()
                                                    messagebox.showinfo("Success", "All photos have been collected", parent = first) 
                                                else:
                                    ...
                                    if len(contact_var.get()) != 10:
                                        ...
                                        if(re.search('', contact_var.get())):
                                            ...
                                            if(re.search(regex, address_var.get())):
                                                ...
                                                student_ids = [x.split('_')[0] for x in student_names]
                                                if id in student_ids:
                                                    ...
                                                    if (q == True):
                                                        ...
                                                        shutil.rmtree(input_directory) 
                                                        output_directory = os.path.join(dataset_dir,name)
                                                        ...
                                                        video_capture.release()
                                                        cv2.destroyAllWindows()
                                                        cur.execute("update attendance set mojar = %s, fname = %s, gender = %s, contact_no = %s, email_address = %s where sid = %s", (                                                               
                                                                                                    mojar_var.get(),
                                                                                                    fname_var.get(),
                                                                                                    gender_var.get(),
                                                                                                    contact_var.get(),
                                                        ...
                                                        os.rename(os.path.join(dataset_dir,student_name),os.path.join(dataset_dir,name))
                                                        cur.execute("update attendance set mojar = %s, fname = %s, gender = %s, contact_no = %s, email_address = %s where sid = %s", (                                                               
                                                                                                                        mojar_var.get(),
                                                                                                                        fname_var.get(),
                                                                                                                        gender_var.get(),
                    ...
                    # Student Management form 
                        f2 = Frame(first, bg = "skyblue",borderwidth = "3", relief = SUNKEN, height = 600, width = 420)
                        titles = Label(f2, text = "Manage student" ,bg = "skyblue", font = ("Italic", 20, "bold")).place(x = 90, y = 30)

                        id = Label(f2, text = "Student ID", bg = "skyblue", font = ("italic",13, "bold")).place(x = 35, y = 100 )
                        E1 = Entry(f2,state="disabled", width = 20, textvariable = sid_var,  font = ("italic",13, "bold") ).place(x = 180  , y = 100)
                        
                        mojar = Label(f2, text = "mojar", bg = "skyblue",  font = ("italic",13, "bold")).place(x = 35, y = 150 )
                        E2 = Entry(f2, width = 20, textvariable = mojar_var,  font = ("italic",13, "bold")).place(x =180, y = 150)

                        name = Label(f2, text = "Full Name", bg = "skyblue", font = ("italic",13, "bold")).place(x =35, y = 200)
                        E3 = Entry(f2, width = 20, textvariable = fname_var , font = ("italic",12, "bold")).place(x = 180, y = 200)

                        gender = Label(f2, text = "Gender", bg = "skyblue", font = ("italic",12, "bold")).place(x = 35, y= 250)
                        E7 = Combobox(f2, textvariable = gender_var , values = ["Male","Female","Others"], state = "readonly",  font = ("italic",11, "bold")).place(x = 180, y = 250)
                        
                        no = Label(f2, text = "Contact.No", bg = "skyblue", font = ("italic",12, "bold")  ).place(x = 35, y = 300)
                        E4 = Entry(f2, width = 20, textvariable = contact_var , font = ("italic",12, "bold") ).place(x = 180, y = 300 ) 

                        address = Label(f2, text = " Email Address", bg = "skyblue", font = ("italic",12, "bold")).place(x = 35, y = 350)
                        ...
                        btn1 = Button(f3, text = "Add", bg = "skyblue", height = "1", width = "7",command = add_student, font = ("Times new Roman", 14 , "bold")).place(x = 10, y = 10)
                        btn2 = Button(f3, text = "Update", bg = "skyblue", height = "1", width = "7", command = update, font = ("Times new Roman", 14 , "bold")).place(x = 105, y = 10)
                        btn3 = Button(f3, text = "Delete", bg = "skyblue",  height = "1", width = "7", command = delete,  font = ("Times new Roman", 14 , "bold")).place(x = 205, y = 10)
                        btn4 = Button(f3, text = "Clear", bg = "skyblue", height = "1", width = "7", command = clear, font = ("Times new Roman", 14 , "bold")).place(x = 305, y = 10)
                        
                        f3.place(x = 20, y = 550)
                    ## Large Frame
                        f4 = Frame(first, height = 600, width = 900, bg = "Skyblue", borderwidth = "3", relief = SUNKEN)
                        f4.place(x = 440, y = 90)
                        l1 = Label(first, text = "Search By:",font = ("times new roman", 18 ,"bold"),bg = "skyblue", fg = "black").place(x = 460, y = 100 )
                        c1 = Combobox(first, textvariable = search_from, values = ["sid","fname","mojar"], state = "readonly", width = "25").place(x = 580, y = 109)
                    ...
                    ## Table frame
                        f5 = Frame(f4, bg = "Skyblue", borderwidth = "2", relief = SUNKEN)
                        f5.place(x = 20, y = 45, height = 550, width = 855 )
                        scroll_x =Scrollbar(f5, orient = HORIZONTAL)
                        scroll_y = Scrollbar(f5, orient = VERTICAL)
                        table1 = Treeview(f5, columns = ("sid","mojar", "fname","gender","contact.no","address","DOJ"), xscrollcommand = scroll_x.set, yscrollcommand = scroll_y.set)
                        scroll_x.pack(side = BOTTOM, fill = X )
                        scroll_y.pack(side = RIGHT, fill = Y)
                        scroll_x.config(command = table1.xview)
                        scroll_y.config(command = table1.yview)
                        table1.heading("sid", text ="student ID")
                        table1.heading('mojar', text = "mojar")
                ...
                def train(): 
                    try:
                        second = Toplevel()
                        second.title("Train The AI System")
                        ...
                        backgrd.pack()
                        train_title = Label(second, text = "Train the AI System", fg = 'black', font = ("times new roman", 20, "bold"), bg = "skyblue")
                        ...
                        btn = Button(second, text = "Start The Training", fg = 'black',font = ("Times new roman", 20, "bold"), command = progress, bg = "skyblue" )
    ...
    ### Function to recognize the face
                ...
                def change():
                    ...
                    title = Label(account_frame, text = "Admin Account", font = ('times new roman', 20, 'bold') , fg = 'black', bd = 3, relief = SUNKEN)    
                    ...
                    btn = Button(account_frame, text = 'Reset', font = ('times new roman', 14, 'bold'), width = 10 , bg = 'skyblue', command = user_change, relief = GROOVE).place(x = 240, y = 380 )
                    account.mainloop()   
                ...
                def report():
                    ...
                    report.config(bg = "skyblue")
                    title = Frame(report, bg = "skyblue", bd = "3", relief = SUNKEN )
                    title.pack(fill = BOTH)
                    title_label = Label(title, text = "Attendance Report", font = ("times new roman", 30, "bold"), fg = "black", bg = "skyblue")
                    title_label.pack()
                    def back():
                        report.destroy()
                    backbtn = Button(title, text = 'Back' , bg = "skyblue" , fg = "black", font = ("Times New Roman", 20 ,"bold"), relief = RIDGE, command = back).place(x = 1250, y = 0)
                    ...
                    search_text = StringVar()
                    # Textfill 
                    text_fill = Frame(report, height = 620, width = 1350, bg= "skyblue", borderwidth = "3", relief = SUNKEN)
                    text_fill.place(x = 10, y = 75)
                    search_label = Label(text_fill, text = "Search By:", font = ("times new roman", 15, "bold"), bg = "skyblue")
                    ...
                    search_today.place(x = 750, y = 10 )
                    show_btn = Button(text_fill,  height = "1", text = "Show All", font = ("times new roman", 15, "bold"), command = show_data, width = 15)
                    show_btn.place(x = 950, y = 10)
                    show_btn = Button(text_fill,  height = "1", text = "export The Report", font = ("times new roman", 15, "bold"), command = export_data, width = 15)
                ...
                def face_embedding():
                    ...
                    backgrd.pack()
                    embed_title = Label(fe, text = "Extract And Save Embeddings",font = ("times new roman", 30, "bold"), bg = "skyblue")
                    ...
                    if not os.path.exists(embeddings_model_file):
                        [image_ids,image_paths,image_arrays,names,face_ids] = embedding_obj.get_all_face_pixels(student_details)
                ...
                topic = Label(attendance, text = manage , bg = "skyblue" , fg = "Black", padx = 15, pady = 15, font = ("Times New Roman", 20, "bold") ,borderwidth = 5, relief = RIDGE)
                topic.place (x = 0, y = 0,relwidth = 1)
                # faceslider()
                # faceslidercolor()

                photo1 = PhotoImage(file = "Photos/management.png", master = attendance)
                B1 = Button(attendance, image = photo1, text = "Student Management",font = ("Times New Roman" , 20, "bold" ), fg = "black", height =230, width = 265, command = manage_student, compound = BOTTOM )
                B1.place(x = 20 , y = 100)

                photo2 = PhotoImage(file = "Photos/face_recognizer.png",  master = attendance)
                B2 = Button(attendance, image = photo2 , text = "Take Attendacne", font = ("Times new roman", 20, "bold"), fg = "black", height = 230, width= 265, command = face_recognize, compound = BOTTOM )
                B2.place(x = 20 , y = 400)
                photo3 = PhotoImage(file = "Photos/train.png",  master = attendance)
                B3 =  Button(attendance , image = photo3 , text = "Train the Data" , font = ("Times new roman", 20, "bold"), fg = "black" , height = 230, width= 265, command = train , compound = BOTTOM )
                B3.place(x = 1040, y = 100)
                photo4 = PhotoImage(file = "Photos/exit1.png",  master = attendance )
                B4 = Button(attendance, text="LogOut",image = photo4, fg = "black",font = ("Times new Roman", 20, "bold"), height = 230, width = 265 , command = exit, compound = BOTTOM)
                B4.place(x =1040, y = 400)
                photo5 = PhotoImage(file = "Photos/report.png" ,  master = attendance)
                B5 = Button(attendance, text = "Attendance Report", fg = "black", font = ("Times new roman", 20, "bold"), image = photo5, height = 230, width = 265, command = report, compound = BOTTOM)
                B5.place(x = 360, y = 400)
                photo6 = PhotoImage(file = "Photos/photosample.png",  master = attendance)
                B6 = Button(attendance, text = "Photo Samples" ,fg = "black", font =("Times new roman",20, "bold"), image = photo6, height = 230, width = 265, command = photo_samples, compound = BOTTOM )
                B6.place(x = 360, y= 100) 
                photo7 = PhotoImage(file = "Photos/passwordchange.png", master = attendance)
                B7 = Button(attendance, text="Admin Account" ,fg = "black",font =("Times new roman",20, "bold"), image = photo7, height = 230, width = 265, command = change, compound = BOTTOM )
                B7.place(x = 700, y = 400)
                photo8 = PhotoImage(file = "Photos/embeddings.png", master = attendance)
                B8 = Button(attendance, text = "Extract Embeddings", fg = "black", font = ("Times new Roman", 20, "bold"), image = photo8, height = 230, width = 265,command= face_embedding, compound = BOTTOM)
                B8.place(x = 700, y =100)
                attendance.mainloop()
        except pymysql.err.OperationalError as e:
            messagebox.showerror( "Error","Sql Connection Error... Open Xamp Control Panel and then start MySql Server ")
        except Exception as e:
...
title = Label(face, text = "Smart Attendance Management System \n Admin Login Page" , font = ("times new roman", 30, "bold"), bg = "skyblue", fg = "black", bd = 7, relief = GROOVE) 
title.place(x = 0, y = 0, relwidth = 1)

clock = Label(face , font = ("times",20,"bold"), bg = "skyblue", relief = GROOVE)
...
submit_btn = Button(login_frame, text = "Log In",width = 10, activebackground = "blue", activeforeground = "white", command = login , font = ("times new roman", 20, "bold"),relief = GROOVE, bg = "skyblue").grid(row = 3, column = 1, pady =25, padx = 25) 
face.mainloop()            